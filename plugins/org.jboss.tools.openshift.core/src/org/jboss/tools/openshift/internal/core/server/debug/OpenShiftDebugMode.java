...
public class OpenShiftDebugMode {
	...
	private static final String ROUTE_DEBUG_TIMEOUT = "42h";
	...
	private void toggleTimeout(IResource resource, Connection connection, DebugContext context, IProgressMonitor monitor) {
		if (context.isDebugEnabled()) {
			...
			clearRouteTimeout(resource, connection, monitor);
		}
	}
	...
	private void setRouteTimeout(IResource resource, Connection connection, IProgressMonitor monitor) {
		...
		if (!hasTimeoutAnnotation(route)) {
			route.setAnnotation(OpenShiftAPIAnnotations.TIMEOUT, ROUTE_DEBUG_TIMEOUT);
			connection.updateResource(route);
		}
	}

	private void clearRouteTimeout(IResource resource, Connection connection, IProgressMonitor monitor) {
		monitor.subTask("Removing route timeout...");

		IRoute route = getRoute(resource, connection, monitor);
		if (route == null) {
			OpenShiftCoreActivator.pluginLog().logInfo(
					NLS.bind("Could not remove timeout after debugging: could not find any route for resource {0}", resource.getName()));
			return;
		}

		OpenShiftCoreActivator.pluginLog().logInfo(
				NLS.bind("Removing haproxy timeout for route {0}", route.getName()));
		if (hasTimeoutAnnotation(route)) {
			// dont remove obviously user set timeouts
			route.removeAnnotation(OpenShiftAPIAnnotations.TIMEOUT);
			connection.updateResource(route);
		}
	}

	private boolean hasTimeoutAnnotation(IRoute route) {
		String timeout = route.getAnnotation(OpenShiftAPIAnnotations.TIMEOUT);
		return ROUTE_DEBUG_TIMEOUT.equals(timeout);
	}
